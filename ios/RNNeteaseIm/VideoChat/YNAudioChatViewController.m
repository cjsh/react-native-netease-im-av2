...
//    self.connectingLabel.text   = @"正在呼叫，请稍候...";
    self.userIconImageView.hidden = YES;
    NSString *nick = [NTESSessionUtil showNick:self.callInfo.callee inSession:nil];
    self.connectingLabel.text = [NSString stringWithFormat:@"正在呼叫%@，请稍候...",nick];
...
- (void)audioCallingInterface{
    ...
    self.userIconImageView.hidden = NO;
    self.hangUpBtn.hidden  = NO;
    ...
    NSURL *imageURL = [NSURL URLWithString:user.userInfo.avatarUrl];
...
- (UIImageView *)userIconImageView {
    if (!_userIconImageView) {
        ...
        _userIconImageView.frame = CGRectMake(15, 74, 92, 92);
    }
