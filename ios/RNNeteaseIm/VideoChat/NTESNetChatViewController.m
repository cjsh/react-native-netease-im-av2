...
@property (nonatomic, assign) int duration_time;
...
- (void)doStartByCaller
{
    ...
    __weak typeof(self) wself = self;
    [[NERtcCallKit sharedInstance] call:self.callInfo.callee type:wself.callInfo.callType attachment:@"音视频请求扩展信息" completion:^(NSError * _Nullable error) {
        if (!error && wself) {
//            wself.callInfo.callID = callID;
            ...
            dispatch_after(dispatch_time(DISPATCH_TIME_NOW, (int64_t)(delayTime * NSEC_PER_SEC)), dispatch_get_main_queue(), ^{
                [wself onControl:wself.callInfo.callID from:wself.callInfo.callee type:NIMNetCallControlTypeFeedabck];
            });
        }else{
            if (error) {
...

{
    ...
    _myTimer = [NSTimer scheduledTimerWithTimeInterval:1.0f target:self selector:@selector(timerAction:) userInfo:nil repeats:YES];
    NSLog(@"……………………setAvDuration……………………");
}
