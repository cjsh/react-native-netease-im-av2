...
-(void)addMembers:(NSString *)teamId accounts:(NSArray *)count Succ:(Success)succ Err:(Errors)err{
    ...
    [[NIMSDK sharedSDK].teamManager addUsers:count toTeam:teamId postscript:postscript attach:nil completion:^(NSError * _Nullable error, NSArray<NIMTeamMember *> * _Nullable members) {
        if (!error) {
            succ(@"200");
        }else{
