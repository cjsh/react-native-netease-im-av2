...
@interface RNNotificationCenter () <NIMSystemNotificationManagerDelegate,NIMChatManagerDelegate, NIMNetCallManagerDelegate, NERtcCallKitDelegate>
...
- (void)onInvited:(NSString *)invitor userIDs:(NSArray<NSString *> *)userIDs isFromGroup:(BOOL)isFromGroup groupID:(NSString *)groupID type:(NERtcCallType)type attachment:(NSString *)attachment{
    NSString *localUser = [[NIMSDK sharedSDK].loginManager currentAccount];
    [NIMSDK.sharedSDK.userManager fetchUserInfos:@[invitor, localUser] completion:^(NSArray<NIMUser *> * _Nullable users, NSError * _Nullable error) {
        if (error) {
            return;
        }else {
            //远端对象
...
//    if ([self shouldFireNotification:invitor]) {
//        NSString *text = [self textByCaller:invitor
//                                       type:type];
...
//            vc = [[YNVideoChatViewController alloc] initWithCaller:invitor callId:1];
//        }
//            break;
//        case NIMNetCallTypeAudio:{
//            vc = [[YNAudioChatViewController alloc] initWithCaller:invitor callId:1];
//        }
//            break;
//        default:
//            break;
...
//    NSString *content = notification.content;
    ...
    if (notiDict){
        //本app中只有普通的多人音视频会话
        NSString *teamId = [notiDict objectForKey:@"teamId"];
        NSInteger notiType = [[notiDict objectForKey:@"type"] integerValue];
        if (teamId) {
            [[NSNotificationCenter defaultCenter] postNotificationName:@"meeting_YN" object:nil userInfo:notiDict];
        }else{
            switch (notiType) {
                case 1://加好友
