...
@property(strong,nonatomic) UIImageView *noVideoBackImage;// 关闭视频采集时的背景展示
...
@property(assign, nonatomic) BOOL isRemoteOpenCamera;//对方是否开启摄像头
...
@property (nonatomic, strong) NSNumber *hangupType;//挂断情况
@property (nonatomic, assign) int duration_time;//总共视频时间
...
- (void)viewDidLoad {
    ...
    self.hangupType = @1;
    self.duration_time = -100;
    self.isRemoteOpenCamera = YES;
    [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(setAvDuration:) name:@"ios_duration" object:nil];
    [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(showAlertMoney) name:@"payNotice" object:nil];
...
- (void)setupUI {
    ...
    self.noVideoBackImage = [[UIImageView alloc] init];
    [self.view addSubview:self.noVideoBackImage];
    self.noVideoBackImage.contentMode = UIViewContentModeScaleAspectFit;
    [self.noVideoBackImage mas_makeConstraints:^(MASConstraintMaker *make) {
        make.edges.equalTo(self.view);
    }];
    [self.noVideoBackImage sd_setImageWithURL:[NSURL URLWithString:self.remoteUser.avatar]];
    self.noVideoBackImage.hidden = true;
...
- (void)updateUIonStatus:(NERtcCallStatus)status {
    switch (status) {
        ...
        {
            self.titleLabel.text = [NSString stringWithFormat:@"正在呼叫 %@",self.remoteUser.nickName];
        ...
        {
            self.titleLabel.text = [NSString stringWithFormat:@"%@",self.remoteUser.nickName];
            self.centerTitleLabel.text = self.titleLabel.text;
            self.remoteAvatorView.hidden = NO;
            [self setUrl:self.remoteUser.avatar withPlaceholder:@"avator"];
            [self.remoteBigAvatorView sd_setImageWithURL:[NSURL URLWithString:self.remoteUser.avatar] placeholderImage:[UIImage imageNamed:@"avator"]];
        ...
        case NERtcCallStatusInCall:
        {
            ...
            self.titleLabel.text = self.remoteUser.nickName;
            self.centerTitleLabel.text = self.remoteUser.nickName;
        }
            break;
        default:
            break;
...
- (void)cameraBtnClick:(UIButton *)button {
    ...
    if (self.isRemoteOpenCamera != YES){
        //关闭摄像头
        self.noVideoBackImage.hidden = false;
    }
}
...
- (void)onDisconnect:(NSError *)reason {
    ...
    [self destroy];
    //无网络中途挂断
    [[NSNotificationCenter defaultCenter] postNotificationName:@"EventReminder" object:nil userInfo:@{@"code":@"6",@"sendName":@"VIDEO_CALL_CODE"}];
}
- (void)onUserEnter:(NSString *)userID {
    ...
    [self startTimer];
    //通话开始
    [[NSNotificationCenter defaultCenter] postNotificationName:@"IMConnect" object:nil userInfo:@{@"isSuccess":@"true",@"sendName":@"VIDEO_CALL_SUCCESS"}];
}
...
- (void)onCameraAvailable:(BOOL)available userID:(NSString *)userID {
    NSLog(@"onCameraAvailable>>>>>>>>%d",available);
...
- (void)onVideoMuted:(BOOL)muted userID:(NSString *)userID {
    ...
    NSLog(@"onVideoMuted>>>>>>>>%d",muted);
    
    self.isRemoteOpenCamera = !muted;
...
//    }
    ...
    [[NSNotificationCenter defaultCenter] postNotificationName:@"EventReminder" object:nil userInfo:@{@"code":@"2",@"sendName":@"VIDEO_CALL_CODE"}];
}
...
- (void)onUserReject:(NSString *)userID {
    ...
    [self destroy];
    //对方拒绝接听
    [[NSNotificationCenter defaultCenter] postNotificationName:@"EventReminder" object:nil userInfo:@{@"code":@"-1",@"sendName":@"VIDEO_CALL_CODE"}];
}
...
- (void)setAvDuration:(NSNotification *)notification
{
//    if (self.timer != nil) {
//        //取消定时器
...
//    [self startTimer];
    ...
    _duration_time = [duration intValue];
...
- (void)showAlertMoney{
    ...
    __weak typeof(self) weakSelf = self;
    UIAlertController *alert = [UIAlertController alertControllerWithTitle:@"温馨提示" message:@"您的余额还剩余5分钟的问诊时间，请立即充值，5分钟后问诊自动结束" preferredStyle:UIAlertControllerStyleAlert];
    ...
    UIAlertAction *defaultAction = [UIAlertAction actionWithTitle:@"立即充值" style:UIAlertActionStyleDefault handler:^(UIAlertAction * _Nonnull action) {
        //挂断
       
        [weakSelf hangup];
        [[NSNotificationCenter defaultCenter] postNotificationName:@"EventReminder" object:nil userInfo:@{@"code":@"-1",@"sendName":@"AV_RECHARGE_AMOUNT"}];
...
- (void)figureTimer {
    ...
    if (_duration_time != -100){
        //说明开启了计时器
        if (_duration_time == 300) {
            [[NSNotificationCenter defaultCenter] postNotificationName:@"EventReminder" object:nil userInfo:@{@"code":@"-1",@"sendName":@"VIDEO_CALL_FIVE_MINUTE"}];
            
           
        }else if (_duration_time == 0){
            __weak typeof(self) weakSelf = self;
            //挂断
            [weakSelf hangup];
            //挂断事件监听
...
- (void)hangup{
    ...
    if ([self.hangupType isEqual:@2]) {//费用不足挂断
        [[NSNotificationCenter defaultCenter] postNotificationName:@"EventReminder" object:nil userInfo:@{@"code":@"2",@"sendName":@"VIDEO_CALL_CODE"}];
    }else{
        //挂断事件监听
        [[NSNotificationCenter defaultCenter] postNotificationName:@"EventReminder" object:nil userInfo:@{@"code":@"-1",@"sendName":@"VIDEO_CALL_CODE"}];
...
- (void)destroy {
    ...
    if (self.isCaller == YES && [self.delegate respondsToSelector:@selector(didEndCallWithStatusModel:)]) {
        ...
        self.statusModel.nickName = self.remoteUser.nickName;
