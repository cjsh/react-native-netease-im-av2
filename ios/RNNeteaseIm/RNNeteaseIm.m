...
//手动登录
RCT_EXPORT_METHOD(login:(nonnull NSString *)account token:(nonnull NSString *)token userAvatar:(nonnull NSString *) userAvatar
...
RCT_EXPORT_METHOD(callAvChat:(NSDictionary *)params){
    ...
    NSString *localUser = [[NIMSDK sharedSDK].loginManager currentAccount];
    [NIMSDK.sharedSDK.userManager fetchUserInfos:@[sessionId, localUser] completion:^(NSArray<NIMUser *> * _Nullable users, NSError * _Nullable error) {
        if (error) {
            return;
        }else {
            //远端对象
            ...
            NEUser *localUser = [[NEUser alloc] init];
            if (users.count >= 2){
                NIMUser *imUser = users.lastObject;
                localUser.imAccid = imUser.userId;
                localUser.mobile = imUser.userInfo.mobile;
                localUser.nickName = imUser.userInfo.nickName;
                localUser.avatar = imUser.userInfo.avatarUrl;
            }
            
            NECallViewController *callVC = [[NECallViewController alloc] init];
            callVC.localUser = localUser;
            callVC.remoteUser = remoteUser;
            callVC.status = NERtcCallStatusCalling;
            callVC.callType = [chatType isEqualToString:@"1"] ? NERtcCallTypeAudio : NERtcCallTypeVideo;
            callVC.modalPresentationStyle = UIModalPresentationOverFullScreen;
            UIViewController *rootViewController = RCTPresentedViewController();
            [rootViewController presentViewController:callVC animated:YES completion:nil];
...
// 挂断
RCT_EXPORT_METHOD(hangupAvChat:(NSDictionary *)params){
...
//发送自定义消息
RCT_EXPORT_METHOD(sendTextCustomMessage:(nonnull  NSString *)digest extra:(NSString *)content ){
    

    [[ConversationViewController initWithConversationViewController] sendCustomTextMessage:digest extra:content];
}
//发送视频消息
...
//发送音视频通话时长
RCT_EXPORT_METHOD(setAvDuration:(nonnull NSNumber *)duration){
    NSLog(@"setAvDuration=========> %d", [duration intValue]);
    if ([duration intValue] > 0) {
        
        //通话时长的发送
        [[NSNotificationCenter defaultCenter] postNotificationName:@"ios_duration" object:nil userInfo:@{@"duration":duration}];
...
//调用支付
RCT_EXPORT_METHOD(getPayEvent){

    //通话时长的发送
    [[NSNotificationCenter defaultCenter] postNotificationName:@"payNotice" object:nil userInfo:nil];
