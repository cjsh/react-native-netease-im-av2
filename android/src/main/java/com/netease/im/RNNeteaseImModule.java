...
public class RNNeteaseImModule extends ReactContextBaseJavaModule implements LifecycleEventListener, ActivityEventListener {
     ...
     */
    @ReactMethod
    public void login(String contactId, String token, String userAvatar, final Promise promise) {
        LogUtil.w("cj", "id:_" + contactId);
        LogUtil.w("cj", "t:" + token);
        LogUtil.w("cj", "userAvatar:" + userAvatar);
//        LogUtil.w(TAG, "md5:" + MD5.getStringMD5(token));
        ...
        LoginService.getInstance().login(new LoginInfo(contactId, token), new RequestCallback<LoginInfo>() {
            ...
            public void onSuccess(LoginInfo loginInfo) {
                if (loginInfo != null) {
                    CallKitUI.INSTANCE.setCurrentUserAccId(loginInfo.getAccount());
                    Map<UserInfoFieldEnum, Object> map = new HashMap<>();
                    map.put(UserInfoFieldEnum.AVATAR, userAvatar);
                    NIMClient.getService(UserService.class).updateUserInfo(map);
//                    AVChatKit.setAccount(loginInfo.getAccount());
     ...
     */
    @ReactMethod
    public void getLoginStatus(final Promise promise) {
        int status = NIMClient.getStatus().getValue();
        ...
        NIMClient.getService(TeamService.class).createTeam(fieldsMap, teamTypeEnum, "", array2ListString(accounts))
                .setCallback(new RequestCallback<CreateTeamResult>() {
                    ...
                    public void onSuccess(CreateTeamResult result) {
                        promise.resolve(result.getTeam().getId());
                    }
                    ...
                    public void onException(Throwable exception) {
                        promise.reject("", "创建" + finalTeamName + "异常");
...
//                promise.resolve(ReactCache.createMessage(message,null));
     ...
     */
    @ReactMethod
    public void sendTextCustomMessage(String content, String extra, final Promise promise) {
        LogUtil.w(TAG, "sendTextMessage" + content);

        sessionService.sendTextCustomMessage(content,extra, new SessionService.OnSendMessageListener() {
            @Override
            public int onResult(int code, IMMessage message) {
//                promise.resolve(ReactCache.createMessage(message,null));
                return 0;
