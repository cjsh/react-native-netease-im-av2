...
class Session {
     ...
     */
    login(contactId, token, userAvatar) {
        return RNNeteaseIm.login(contactId, token, userAvatar)
    }
    /**
    * ios调用视频通话
    */
    ...
    setAvChatDuration = (duration) => {
        RNNeteaseIm.setAvDuration(duration)
    }

    showRechargeAmountDialog = () => {
        RNNeteaseIm.getPayEvent()
    }
     ...
     */
    getStatus() {
        return RNNeteaseIm.getLoginStatus();
    }
     ...
     * @param atUserIds @的群成员ID ["abc","abc12"]
     */
    sendTextCustomMessage(content, extra){
        return RNNeteaseIm.sendTextCustomMessage(content, extra)
    }
